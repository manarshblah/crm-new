...
export const LeadsPage = () => {
    ...
    return (
        <PageWrapper 
            ...
            actions={
                <>
                    <Input id="search-leads" placeholder={t('search')} className="ps-10 w-full sm:w-auto" icon={<SearchIcon className="w-4 h-4" />} />
                ...
                {leadStatusFilters.map(status => {
                    ...
                    return (
                        <button 
                            ...
                            className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium whitespace-nowrap flex-shrink-0 ${activeStatusFilter === status ? 'border-b-2 border-primary text-primary' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                        >
                            ...
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 min-w-[800px]">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-3 sm:px-6 py-3 hidden md:table-cell">{t('lastFeedback')}</th>
                                        <th scope="col" className="px-3 sm:px-6 py-3 hidden lg:table-cell">{t('notes')}</th>
                                        <th scope="col" className="px-3 sm:px-6 py-3 hidden md:table-cell">{t('lastStage')}</th>
                                        <th scope="col" className="px-3 sm:px-6 py-3 hidden lg:table-cell">{t('reminder')}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredLeads.map(lead => (
                                        <tr key={lead.id} className="bg-white dark:bg-dark-card border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td className="px-3 sm:px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{lead.name}</td>
                                            ...
                                            <td className="px-3 sm:px-6 py-4 hidden md:table-cell">{lead.lastFeedback}</td>
                                            <td className="px-3 sm:px-6 py-4 hidden lg:table-cell max-w-xs truncate">{lead.notes}</td>
                                            <td className="px-3 sm:px-6 py-4 hidden md:table-cell">{lead.lastStage}</td>
                                            <td className="px-3 sm:px-6 py-4 hidden lg:table-cell">{lead.reminder}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
