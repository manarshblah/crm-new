...
type ButtonProps = { children: ReactNode, onClick?: () => void, className?: string, variant?: 'primary' | 'secondary' | 'ghost' | 'danger', disabled?: boolean, loading?: boolean };
export const Button = ({ children, onClick, className = '', variant = 'primary', disabled, loading }: ButtonProps) => {
  ...
  return <button onClick={onClick} className={`${baseClasses} ${variantClasses[variant]} ${className}`} disabled={disabled || loading}>{loading ? <Loader className="h-5" /> : children}</button>;
};